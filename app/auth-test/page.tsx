...
export default function AuthTestPage() {
  ...
  const fetchOrCreateUser = useCallback(async () => {
    ...
    try {
      ...
      console.group("🔍 [fetchOrCreateUser] 사용자 데이터 조회/생성 시작");
      ...
      console.log("조회 결과:", { data, error: fetchError });
      ...
      // 사용자가 없으면 생성
      if (!data) {
        console.log("사용자 데이터 없음, 새로 생성 시도");
        const userName =
          user.fullName ||
        ...
        console.log("생성 결과:", { newUser, error: createError });
      ...
      console.group("❌ [fetchOrCreateUser] 에러 발생");
      console.error("에러 타입:", typeof err);
      console.error("에러 객체:", err);
      if (err && typeof err === "object") {
        console.error("에러 키:", Object.keys(err));
        if ("code" in err) console.error("에러 코드:", err.code);
        if ("message" in err) console.error("에러 메시지:", err.message);
        if ("details" in err) console.error("에러 상세:", err.details);
        if ("hint" in err) console.error("에러 힌트:", err.hint);
      }
      console.error("직렬화된 에러:", JSON.stringify(err, null, 2));
      console.groupEnd();
